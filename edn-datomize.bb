...
;; com-etzhayyim-mitsuho 用に移植（schema-path を repo root に調整 + ns-preserving
;; モード wrap-map-keep-ns を追加。Phase 3 EDN-datomize fanout、2026-07-10）。
;;
...
;; トップレベルの entity+attribute 粒度でのクエリは常に有効、blob の中身は
;; 呼び出し側で edn/read-string すれば読める）。属性定義は schema.edn に自動
;; 登録する（Datomic/Datascript 両対応、:db.install/_attribute 等の Datomic
;; 固有キーは使わない）。
;;
;; 使い方:
...
;; com-etzhayyim-mitsuho adaptation: this child repo has no manifest/ dir —
;; schema.edn accumulates at the repo root (ADR-2607101600-series Phase 3 fanout).
...
(defn merge-schema! [new-attrs]
  (let [existing (load-schema)
    ...
    (spit (schema-path) (str ";; schema.edn — Datomic/Datascript 互換スキーマ定義（自動生成 by edn-datomize.bb）\n"
                              ...
                              ";; 手編集禁止 — 再生成すると上書きされる。\n"
                              ";;\n"
                              ";; com-etzhayyim-mitsuho Phase 3 fanout: cells/*.edn は既に名前空間付き\n"
                              ";; キー(:cell/*)だったため wrap-map-keep-ns で既存名前空間を保持。\n"
                              ";; lex/*.edn は裸キー(:lexicon/:id/:defs)だったためファイル単位の\n"
                              ";; 名前空間(lex.<lexiconName>)を新規付与した。manifest.edn は skip\n"
                              ";; （G1 gate/rule に未エスケープの引用符があり edn/read-string が\n"
                              ";; 静かに誤パースする pre-existing corruption。未修正のまま残置）。\n\n"
                              (pr-str merged)
                              "\n"))
    merged))
...
;; ---------- namespace-preserving wrap (mitsuho manifest.edn / cells/*.edn) ----------
;;
;; Some files already use idiomatic namespaced keys (:actor/id, :cell/id, ...) —
;; re-prefixing with a new file-level namespace would destroy the existing
;; meaningful namespace. This mode keeps any key that already has a namespace
;; as-is, and only prefixes genuinely bare keys with ns-name (fallback, unused
;; by mitsuho's files but kept for parity/future files).

(defn keep-or-namespace-key [ns-name k]
  (if (namespace k) k (keyword ns-name (name k))))

(defn entity-from-map-preserve-ns [content ns-name]
  (into {:db/id -1}
        (map (fn [[k v]] [(keep-or-namespace-key ns-name k) (attr-value v)]))
        content))

(defn schema-attrs-preserve-ns [content ns-name]
  (for [[k v] content]
    (let [{:keys [type card]} (classify v)]
      {:db/ident (keep-or-namespace-key ns-name k)
       :db/valueType type
       :db/cardinality card})))
